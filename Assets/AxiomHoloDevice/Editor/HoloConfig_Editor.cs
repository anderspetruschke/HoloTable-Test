...
public class HoloConfig_Editor : Editor
{
  ...
  SerializedProperty VRPNOverTCP;
  ...
  void OnEnable()
  {
    ...
    VRPNOverTCP = serializedObject.FindProperty("VRPNOverTCP");
  ...
  void DrawDeviceOptions()
  {
    ...
    // Ensure Device Manager prefab is valid
    if (DeviceManagerPrefab == null)
    {
      Resources.Load<HoloDeviceManager>("Prefabs/HoloDeviceManager");
      if (DeviceManagerPrefab == null)
      {
        Debug.LogError("The AxiomHoloCave GameObject needs a HoloDeviceManager defined in order to work. Please drag and drop the HoloDeviceManager provided in the Unity Toolkit Assets/Prefab/ folder.");
      }
    }
  }
  ...
  void DrawTrackingOptions()
  {
    ...
    if (EnableTracking.boolValue)
    {
      ...
      EditorGUILayout.PropertyField(VRPNOverTCP);
    }
  }
