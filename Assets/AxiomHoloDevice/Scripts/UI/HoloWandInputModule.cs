...
namespace UnityEngine.EventSystems
{
  public class HoloWandInputModule : PointerInputModule
  {
    ...
    protected Dictionary<int, RaycastResult> m_wandHitResults = new Dictionary<int, RaycastResult>();

    public bool GetWandRayCastResult(int userID, out RaycastResult result)
    {
      return m_wandHitResults.TryGetValue(userID, out result);
    }
    ...
    protected MouseState GetWandEventData(int userID, HoloTrackWand wand)
    {
      ...
      for (int i = 0; i < HoloTrackWand.ButtonsCount; ++i)
      {
        ...
        { // Only raycast for the first button
          ...
          m_wandHitResults.Remove(userID);
          m_wandHitResults.Add(userID, hitResult);
